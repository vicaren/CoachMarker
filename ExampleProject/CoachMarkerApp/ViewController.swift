...
class ViewController: UIViewController {
  ...
  let markerData = [
    CoachMarkerCircleData(coordinate: CGPoint(x: 10, y: 10), radius: 30),
  ...
  func showCoachMarker() {
    coachMarker = CoachMarker(parentView: view, dataSource: self)
    coachMarker?.delegate = self
    coachMarker?.showCoachMarker()
  }
}

// MARK: - CoachMarkerDataSource
